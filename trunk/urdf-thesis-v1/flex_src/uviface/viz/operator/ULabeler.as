package uviface.viz.operator
{
	 ...
	 */
	public class ULabeler extends Operator
	{
		 ...
		 *  data objects)
		 */
		public function ULabeler(source:*=null, group:String=Data.NODES,
			...
			create:Boolean=false, visible:Boolean=true):TextSprite
		{
			...
			// ------------------------------------------
			// new since 09.11.10, by Timm Meiser
			// ------------------------------------------
			var otherChar:Boolean = false;
			//trace("Label is : " + label.text);
			for (var i:int = 0;i<label.text.length; i++) {
				if(label.text.charCodeAt(i) > 255) {
					otherChar = true;
					break;
				}
				  // return;
				//trace("Char at position " + i + " is " + label.text.charAt(i));
				//trace("Charcode at position " + i + " is " + label.text.charCodeAt(i));
			}
			if(otherChar)
