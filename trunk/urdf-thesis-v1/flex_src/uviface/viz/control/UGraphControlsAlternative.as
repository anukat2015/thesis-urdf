package uviface.viz.control
{
	...
	import uviface.ui.application.UApplicationControlBar;
	...
	public class UGraphControls
	{
					...
					if(!UFlareVis.lineageMode) {
						...
						for (i; i<e.node.inDegree; i++) {
							...
							*/
							...
							node.fillColor = UNode.DEFAULT_NODE.fillColor;
							// check for alpha ditance mode
							if(UFlareVis.alphaDistanceFilterActivated) {
								if(node.data.hasOwnProperty("alpha") && node.data.alpha == false) {
								   node.fillAlpha = UFlareVis.distanceAlphaValue;
								}
								else
								   node.fillAlpha = UNode.DEFAULT_NODE.alpha;
							} 
							else
							   node.fillAlpha = UNode.DEFAULT_NODE.alpha;
				...
				function(e:SelectionEvent):void {
					...
					//if(e.object is EdgeSprite) {
						...
						if(!UFlareVis.lineageMode) {
						   ...
						   //e.edge.source.fillColor = UNode.DEFAULT_NODE.fillColor;
						   // check for alpha ditance mode
						   e.edge.source.fillColor = UNode.DEFAULT_NODE.fillColor;
						   e.edge.target.fillColor = UNode.DEFAULT_NODE.fillColor;
						   if(UFlareVis.alphaDistanceFilterActivated) {
							   //e.edge.source.fillColor = UNode.DEFAULT_NODE.fillColor;
						   	   //e.edge.target.fillColor = UNode.DEFAULT_NODE.fillColor;
							   if(e.edge.source.data.hasOwnProperty("alpha") && e.edge.source.data.alpha == false)
							     e.edge.source.fillAlpha = UFlareVis.distanceAlphaValue;							   
							   if(e.edge.target.data.hasOwnProperty("alpha") && e.edge.source.data.alpha == false)
								  e.edge.target.fillAlpha = UFlareVis.distanceAlphaValue;
						   }
						}
						else {
		...
		private static function updateToolTip(event:TooltipEvent):void {
			...
			/*
			if(event.node != null) 
			   TextSprite(event.tooltip).text = event.node.data.tooltip ;
			else if(event.edge != null)
			   TextSprite(event.tooltip).text = event.edge.data.tooltip;//event.edge.data.label;
			else if(event.object is TextSprite)
			   TextSprite(event.tooltip).text = ((event.object as TextSprite).parent as EdgeSprite).data.tooltip;//event.edge.data.label;
			*/
			/*
			if(UFlareVis.lineageMode) {                               
			   //event.tooltip = _lineageToolTip;
			   if(event.node != null) 
			       UStringToolTip(event.tooltip).toolTipText = event.node.data.tooltip;
			   else
			       UStringToolTip(event.tooltip).toolTipText = event.edge.data.tooltip;
			}
			else {
			    if(event.node != null) { 
			        //event.tooltip = _nodeToolTip;
			        UNodeToolTip(event.tooltip).setNodeProperties(event.node); 
			    }
			    else 
			      if(event.edge != null) {
			    	//event.tooltip = _edgeToolTip;
			        UEdgeToolTip(event.tooltip).setEdgeProperties(event.edge);
			    }        
			}
			*/
			                             
			//if(event.edge != null && event.edge.data.selected)
