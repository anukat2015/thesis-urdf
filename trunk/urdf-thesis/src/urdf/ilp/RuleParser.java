...
{
	private static final String triplet = "\\s*\\?{0,2}\\w+\\d*\\s*\\(\\s*((\\s*\\\".*\\\"\\s*)|(\\s*\\?{0,2}\\w+\\d*\\s*))\\s*\\,\\s*((\\s*\\\".*\\\"\\s*)|(\\s*\\?{0,2}\\w+\\d*\\s*))\\s*(\\,\\s*\\d+\\s*){0,1}\\)";
	private static final String id = "\\s*\\?{0,2}\\w+\\d*\\s*";
	private static final String arg = "(\\s*\\\".*\\\"\\s*)|(\\s*\\?{0,2}\\w+\\d*\\s*)"; // only quoted literals or strict alphanumeric arguments!
	...
	public static void main(String[] args) throws Exception {
		...
		BufferedReader in = new BufferedReader(new FileReader("politicianOf.txt"));
		String read;
		USoftRule rule=null;
		...
		while(read!=null)//read a line from file and save into a string
   		{			
			if (read!=null)
			{
				rule=RuleParser.parseSoftRule(read);
