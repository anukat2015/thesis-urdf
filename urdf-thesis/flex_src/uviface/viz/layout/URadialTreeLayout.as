package uviface.viz.layout
{
	 ...
	 * </p>
	 */
	public class URadialTreeLayout extends Layout
	{
		// -- Properties ------------------------------------------------------
	    ...
	    private var _avgDegree:Number = 0;
		 ...
		 */		
		public function URadialTreeLayout(radius:Number=DEFAULT_RADIUS,
		...
		protected override function layout():void
		{
			...
			// this is a hack to use for the radial layout
			// done by Timm Meiser on 12.08.10
			// see line 365 oin the UGraphDistanceFilter class
			/*
			this.visualization.data.nodes.visit(
			      function(n:NodeSprite):void {
			         //t.$(ns).visible = true;
			         if(n.data.hasOwnProperty("Show") && n.data["Show"] == true) {
			         	_t.$(n).visible = true;
			         	_t.$(n).alpha = 1;
			         	_t.$(n).expanded = true;
						_t.$(n).mouseEnabled = true;
						_t.$(n).buttonMode = true;
			         	//trace("radial node is : " + n.data.tooltip);
			         	//trace("radial node alpha is: " + n.alpha);
			         	n.data["Show"] = false;
			         }
			      }
		    );*/
	    	...
	    	theta1:Number, theta2:Number):void
	    {
	        ...
	        for each (var c:NodeSprite in sortedChildren(n)) {
	            ...
	            */
	            ...
	            if (c.expanded && c.childDegree > 0)
	            {
	                // between big node clustern -> longer edges
	                if(c.childDegree >= _avgDegree && n.childDegree >= _avgDegree) 
	                    doLayout(c, r+_radiusInc, theta1 + nfrac*dtheta, 
	                                          theta1 + (nfrac+cfrac)*dtheta);
	                // else -> shorter edges
	                else
	                    //doLayout(c, r+(_radiusInc/2), theta1 + nfrac*dtheta, 
	                      //                    theta1 + (nfrac+cfrac)*dtheta);
	                    doLayout(c, (r/2)+_radiusInc, theta1 + nfrac*dtheta, 
	                                          theta1 + (nfrac+cfrac)*dtheta);
	            }
	            else if (c.childDegree > 0)
	            {
	            	var cr:Number = (c.childDegree >= _avgDegree && n.childDegree >= _avgDegree) 
	            	              ? (r + _radiusInc) : ((r/2) + _radiusInc);
	            	var ca:Number = theta1 + nfrac*dtheta + cfrac*dtheta2;
								...
								aw:Number, v:Boolean) : void
		{
			...
			// new hack from Timm Meiser on 11.08.10
			//o.visible = n.alpha ? 1 : 0;   
			
			// we have to adjust this alpha value so that the alpha value reflects our edge certainty
			if (n.parentEdge != null)
				//_t.$(n.parentEdge).alpha = UMath.roundNumber(parseFloat(n.parentEdge.data.confidence),2);//alpha;
				UEdge.changeEdgeAlpha(n.parentEdge,_t);
		}
		...
		private function computeAVGDegree():void {
			
			_avgDegree = 0;
			for each (var node:NodeSprite in this.visualization.data.nodes) {
				_avgDegree += node.childDegree;
			}
			_avgDegree = (_avgDegree/this.visualization.data.nodes.length);
		}
