package uviface.viz.layout
{
	 ...
	 * @see flare.physics
	 */
	public class UForceDirectedLayout extends Layout
	{
		...
		private static var _refreshRate:int = 1;
		private var _refreshRateCounter:int = 1;
		
		public static function set refreshRate(refreshRate:int):void {
			_refreshRate = refreshRate;
		}
		
		// this is a reduced list of edges only containing one edge
		// between two particular nodes
		/*
		private var _forceEdgeList:Array = new Array();
		
		public function set forceEdgeList(forceEdgeList:Array):void {
			_forceEdgeList = forceEdgeList;
			var edge:EdgeSprite;
			trace("Number of Reduced Edges is : " + _forceEdgeList.length.toString());
			for each (edge in _forceEdgeList) {
				trace("Edge is : " + edge.data.tooltip);
			}
		}
		*/
		 ...
		 */
		public function UForceDirectedLayout(enforceBounds:Boolean=false, 
		...
		protected override function layout():void
		{
			// -----------------------------------
			// new since 04.12.10
			// decrement the refresh rate counter
			_refreshRateCounter--;
			
			if(_refreshRateCounter != 0)
			   return;
			else 
		...
		public var damping:Function = function(e:EdgeSprite):Number {
			...
			//return Spring(e.props.spring).tension / 2;
		}
