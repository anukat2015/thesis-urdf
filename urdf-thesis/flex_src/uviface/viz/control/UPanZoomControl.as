package uviface.viz.control
{
	...
	public class UPanZoomControl extends Control
	{
		 ...
		 */
		public function UPanZoomControl(hitArea:InteractiveObject = null, zoomMin:Number = 0,zoomMax:Number=10000):void
		{
			_hit = hitArea;
        ...
        {
            //modification from cambazz, http://goosebumps4all.net/34all/bb/showthread.php?tid=165
            if (resetInProcess) { return;}
            ...
            //_object.x = stage.stageWidth/2- ((stage.stageWidth/2-oldX)*dz);
            //_object.y = stage.stageHeight/2- ((stage.stage.Height/2-oldY)*dz);
            //_object.x = stage.mouseX - ((stage.mouseX - oldX)*dz);
            //_object.y = stage.mouseY - ((stage.stage.mouseX - oldY)*dz);
            _object.x = hitArea.stage.mouseX - ((hitArea.stage.mouseX - oldX)*dz);
            _object.y = hitArea.stage.mouseY - ((hitArea.stage.stage.mouseY - oldY)*dz);
        }
        
        public function adjustPosition(event:UPanZoomEvent, flareVis:UFlareVis) : void
        {                 
            if (resetInProcess) {
				return;
			}
            ...
            var width:Number = flareVis.visualization.width;
			var height:Number = flareVis.visualization.height;
			var r:Rectangle = event.sourceCanvas.getBounds(event.sourceCanvas);
			...
			if (hasW && !hasH) {
				...
				width  = hasW ? width  : (flareVis.visualization ? flareVis.visualization.width  : r.width);
				height = hasH ? height : (flareVis.visualization ? flareVis.visualization.height : r.height);
			}
			...
			var translatedPoint:Point = mat.transformPoint(event.shiftedPoint);
			var translatedPointOrg:Point = mat.transformPoint(event.orgPoint);
            
			dx = dy = NaN;
			//Displays.panBy(_object, translatedPoint.x - translatedPointOrg.x, translatedPoint.y - translatedPointOrg.y);
			Displays.panBy(_object, translatedPointOrg.x - translatedPoint.x, translatedPointOrg.y - translatedPoint.y);
			mx = translatedPoint.x;
			my = translatedPoint.y;
            
        }
        
        public function zoomPosition(event:UPanZoomEvent, flareVis:UFlareVis) : void
        {
            //modification from cambazz, http://goosebumps4all.net/34all/bb/showthread.php?tid=165
            if (resetInProcess) { return;}
            var dw:Boolean = event.zoomIn;
            var dz:Number = dw ? 1.35 : 0.75;//0.975 : 1.025;
            ...
            if(flareVis.rootNode != null) {
            	_object.x = flareVis.rootNode.x - ((flareVis.rootNode.x - oldX)*dz);
                _object.y = flareVis.rootNode.y - ((flareVis.rootNode.y - oldY)*dz);
            }
            else {
               _object.x = hitArea.stage.mouseX - ((hitArea.stage.mouseX - oldX)*dz);
               _object.y = hitArea.stage.mouseY - ((hitArea.stage.stage.mouseY - oldY)*dz);
