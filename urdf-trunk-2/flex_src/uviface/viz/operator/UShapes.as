package uviface.viz.operator
{
	...
	public class UShapes
	{
		...
		public static const COMPLEX_ROUNDED_RECT:String = "complexRoundedRect";
		...
		private static var _shapes:Object = {
			...
			roundedRect: drawRoundRect,
			complexRoundedRect: drawRoundRectComplex
		};
		...
		/**
		 * Draws a rounded rect shape.
		 * @param g the graphics context to draw with
		 * @param size the (half-)size of the square. The height and width of
		 *  the shape will both be exactly twice the size parameter.
		 */
		/*public static function drawRoundRect(g:Graphics, size:Number, ellpiseWidth:Number = null, ellipseHeight:Number = null):void
		{
			if(ellpiseWidth==null) {
				if(ellipseHeight==null)
				   g.drawRoundRect(-size, -size, 2*size, 2*size, 2, 2);
				else
				   g.drawRoundRect(-size, -size, 2*size, 2*size, 2, ellipseHeight); 
			}
			else
			   if(ellipseHeight==null)
				   g.drawRoundRect(-size, -size, 2*size, 2*size, ellpiseWidth, 2);
			   else
			       g.drawRoundRect(-size, -size, 2*size, 2*size, ellpiseWidth, ellipseHeight);
		}*/
		 ...
		 *  the shape will both be exactly twice the size parameter.
		 */
		public static function drawRoundRect(g:Graphics, rectBounds:Array, ellipseWidth:Number = 30, ellipseHeight:Number = 30):void
		{
			 g.drawRoundRect(rectBounds[0], rectBounds[1], rectBounds[2], rectBounds[3], ellipseWidth, ellipseHeight);
		}
		
		/**
		 * Draws a complex rounded rect shape.
		 * @param g the graphics context to draw with
		 * @param size the (half-)size of the square. The height and width of
		 *  the shape will both be exactly twice the size parameter.
		 */
		public static function drawRoundRectComplex(g:Graphics, rectBounds:Array, topLeftRadius:Number = 0, topRightRadius:Number = 0,
		                        bottomLeftRadius:Number = 0, bottomRightRadius:Number = 0):void
		{	
			g.drawRoundRectComplex(rectBounds[0], rectBounds[1], rectBounds[2], rectBounds[3], topLeftRadius, topRightRadius, bottomLeftRadius, bottomRightRadius);
		}
		 ...
		 *  the shape will both be exactly twice the size parameter.
		 */
		/*public static function drawRoundRectComplex(g:Graphics, size:Number, topLeftRadius:Number = null, topRightRadius:Number = null,
		                        bottomLeftRadius:Number = null, bottomRightRadius:Number = null):void
		{
			...
			g.drawRoundRectComplex(-size, -size, 2*size, 2*size, tl, tr, bl, br);
		}*/
		 ...
		 *  the shape will both be exactly twice the size parameter.
		 */
		public static function drawCrossAt(g:Graphics, x:Number, y:Number, size:Number):void
		{
			g.moveTo(x, y-size);
			g.lineTo(x, y+size);
			g.moveTo(x-size, y);
		 ...
		 *  the shape will both be exactly twice the size parameter.
		 */
		public static function drawXAt(g:Graphics, x:Number, y:Number, size:Number):void
		{
		...
		public static function drawDiamond(g:Graphics, rectBounds:Array):void
		{
			//rectBounds.push(x);//data.size/2);
              // rectBounds.push(y);//ns.data.size/2);
              // rectBounds.push(width);//-ns.data.size/2);
              // rectBounds.push(height)
			g.moveTo(0, (1.5*rectBounds[3]));
			g.lineTo(-(rectBounds[2]/2), 0);
			g.lineTo(0, -(1.5*rectBounds[3]));
			g.lineTo((rectBounds[2]/2), 0);
			g.lineTo(0, (1.5*rectBounds[3]));	
		}
		...
		*/
		public static function drawBSpline(g:Graphics, p:Array, ts:TextSprite, npts:int=-1,
			move:Boolean=true):void
		{
			var N:int = (npts < 0 ? p.length/2 : npts);
			var k:int = N<4 ? 3 : 4, nplusk:int = N+k;
			...
			for (s=0; s <= subdiv; ++s) {
				...
				// added on 27.10.10
				// check to get a point at the middle of the BSpline
				// we have 40 subdivisions -> check for s=20
				if(s==20) {
					ts.x = x;
					ts.y = y;
					//ts.rotation = computeEdgeLabelRotation(p[0],p[1],p[2],p[3]);
				}
			}
		}
		...
		// new by Timm Meiser on 27.10.10
		public static function computeEdgeLabelRotation(x1:Number,y1:Number,x2:Number,y2:Number):Number {
			//var rotation:Number = 0;
			var dy:Number = (x1>x2) ? (y1-y2) : (y2-y1);//y2-y1;
			var dx:Number = (x1>x2) ? (x1-x2) : (x2-x1);
			return UMath.toDegrees(Math.atan2(dy, dx));	//Math.toDegrees(Math.atan2(dy, dx));	
		}
